...
export interface PollinationImageRequest {
  prompt: string;
  model?: string;
  width?: number;
  height?: number;
...
export interface PollinationTextRequest {
  prompt: string;
  model?: string;
  temperature?: number;
  max_tokens?: number;
...
export interface PollinationAudioRequest {
  text: string;
  voice?: string;
  model?: string;
  speed?: number;
...
export interface PollinationImageResult {
  success: boolean;
  data?: {
    url: string;
    prompt: string;
...
export interface PollinationTextResult {
  success: boolean;
  data?: {
    text: string;
    model: string;
...
export interface PollinationAudioResult {
  success: boolean;
  data?: {
    url: string;
    text: string;
...
export interface PollinationModelsResult {
  success: boolean;
  data?: {
    image_models: string[];
    text_models: string[];
...
export class PollinationsService {
  ...
  private textBaseUrl = 'https://text.pollinations.ai';
  
  // Available model configurations
  private imageModels = ['flux', 'kontext', 'turbo', 'playground', 'midjourney'];
  private textModels = ['openai', 'elixposearch', 'claude', 'mistral'];
  private audioVoices = ['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'];

  async generateImage(request: PollinationImageRequest): Promise<PollinationImageResult> {
    try {
      const {
        prompt,
        model = 'flux',
  ...
  async generateText(request: PollinationTextRequest): Promise<PollinationTextResult> {
    try {
      const {
        prompt,
        model = 'openai',
  ...
  async generateAudio(request: PollinationAudioRequest): Promise<PollinationAudioResult> {
    try {
      const {
        text,
        voice = 'nova',
  ...
  async getAvailableModels(): Promise<PollinationModelsResult> {
    try {
      return {
        success: true,
        data: {
  ...
  async imageToImage(prompt: string, imageUrl: string, model: string = 'kontext'): Promise<PollinationImageResult> {
    try {
      if (!prompt || !imageUrl) {
        return {
          success: false,
  ...
  async searchWithPollinationsAI(query: string, model: string = 'elixposearch'): Promise<PollinationTextResult> {
    try {
      if (!query) {
        return {
          success: false,
