...
export class AnimeEnhancedV2Service {
    ...
    userAgent = 'Mozilla/5.0 (compatible; BrokenVZN-API/1.0)';
    ...
    async searchAnime(request) {
      ...
      `;
            ...
            if (request.query)
                variables.search = request.query;
            if (request.genre)
                variables.genre = request.genre;
            if (request.year)
                variables.year = request.year;
            if (request.season)
                variables.season = request.season;
            if (request.status)
                variables.status = request.status;
            if (request.format)
                variables.format = request.format;
            if (request.sort) {
            ...
            const animeList = pageData.media.map((anime) => ({
                ...anime,
    ...
    async getAnimeById(id) {
      ...
      `;
            ...
            const recommendations = anime.recommendations?.nodes?.map((rec) => rec.mediaRecommendation) || [];
            // Process relations
    ...
    async getAiringAnime(page = 1, perPage = 20) {
        return this.searchAnime({
            status: 'RELEASING',
            page,
            perPage,
    ...
    async advancedSearch(filters) {
      ...
      `;
            ...
            if (filters.minScore)
                variables.averageScoreGreater = filters.minScore;
            const response = await axios.post(this.anilistEndpoint, {
                query,
                variables
            }, {
