...
export class WaifuPicsService {
    ...
    async getSfwImage(category = 'waifu') {
        try {
    ...
    async getNsfwImage(category = 'waifu') {
        try {
    ...
    async getBatchImages(category, type = 'sfw', count = 5) {
        try {
            if (count > 20)
            ...
            const promises = Array(count).fill(null).map(() => type === 'sfw' ? this.getSfwImage(category) : this.getNsfwImage(category));
