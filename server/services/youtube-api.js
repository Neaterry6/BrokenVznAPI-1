...
export class YouTubeAPIService {
    ...
    async searchVideos(request) {
        try {
            const { query, maxResults = 25, order = 'relevance', type = 'video', videoDuration, videoDefinition } = request;
            if (!query) {
                return {
                    success: false,
    ...
    async getVideoInfo(videoId, cookies) {
        try {
            if (!videoId) {
                return {
                    success: false,
    ...
    async getChannelInfo(channelId, cookies) {
        try {
            if (!channelId) {
                return {
                    success: false,
    ...
    formatPublishedDate(uploadDate) {
        if (!uploadDate)
            return 'Unknown';
