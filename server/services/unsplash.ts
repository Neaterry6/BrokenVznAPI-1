...
export class UnsplashService {
  ...
  async searchPhotos(request: UnsplashRequest): Promise<UnsplashResult> {
    try {
      const {
        query = 'nature',
        orientation,
      ...
      if (!this.apiKey) {
        const demoPhotos: UnsplashPhoto[] = Array.from({ length: per_page }, (_, i) => ({
          id: `demo-photo-${i + 1}`,
          urls: {
      ...
      const photos: UnsplashPhoto[] = response.data.results.map((photo: any) => ({
        id: photo.id,
        urls: photo.urls,
        user: {
  ...
  async getRandomPhoto(query?: string): Promise<UnsplashResult> {
    try {
      // Demo response if no API key
      if (!this.apiKey) {
        const demoPhoto: UnsplashPhoto = {
