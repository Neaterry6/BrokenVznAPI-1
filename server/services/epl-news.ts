...
export interface EPLNewsRequest {
  limit?: number;
...
export interface EPLNewsResult {
  success: boolean;
  news?: NewsArticle[];
...
export class EPLNewsService {
  async getEPLNews(request: EPLNewsRequest = {}): Promise<EPLNewsResult> {
    try {
      const { limit = 5, includeLiveMatches = true } = request;
      
      const result: EPLNewsResult = {
        success: true,
      ...
      if (includeLiveMatches) {
        try {
          ...
          for (const selector of matchSelectors) {
            if (liveMatches.length >= 5) break;
            
            matches$(selector).each((index, element) => {
              if (liveMatches.length >= 5) return false;
