...
export class Anime2Service {
  ...
  `;
    ...
    async getAniListToken(maxRetries = 3) {
    ...
    async fetchGogoAnimeData(title) {
        try {
            ...
            if (!url)
                return { error: 'No matching Gogoanime data' };
    ...
    async searchAnime(params) {
        try {
            ...
            if (this.aniListToken)
                headers['Authorization'] = `Bearer ${this.aniListToken}`;
            ...
            const resultsWithGogo = await Promise.all(animeList.map(async (anime) => {
