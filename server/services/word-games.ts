...
export interface WordGameRequest {
  game_type?: string;
  difficulty?: string;
  category?: string;
}

export interface WordGameResponse {
  success: boolean;
  game_type: string;
  word?: string;
  challenge?: string;
...
export class WordGamesService {
  ...
  async playWordGame(request: WordGameRequest): Promise<WordGameResponse> {
    try {
      const { 
      ...
      // Get word lists
      const difficultyLevel = this.wordLists[difficulty as keyof typeof this.wordLists] || this.wordLists.medium;
  ...
  private generateAnagram(word: string, difficulty: string): WordGameResponse {
  ...
  private async findRhymes(word: string, difficulty: string): Promise<WordGameResponse> {
    try {
      ...
      if (response.status === 200) {
        ...
        const rhymeWords = rhymes.slice(0, 3).map((r: any) => r.word);
  ...
  private async findSynonyms(word: string, difficulty: string): Promise<WordGameResponse> {
    try {
      ...
      if (response.status === 200) {
        ...
        const synonymWords = synonyms.slice(0, 3).map((s: any) => s.word);
  ...
  private generateRandomWord(word: string, difficulty: string): WordGameResponse {
    return {
      success: true,
      game_type: 'random_word',
      word,
  ...
  private getRandomWord(wordList: string[]): string {
  ...
  private getWordHint(word: string): string {
    return this.hints[word.toLowerCase() as keyof typeof this.hints] || 'No hint available';
  }
