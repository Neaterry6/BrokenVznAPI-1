export interface QuoteResult {
  ...
  quote?: string;
  category?: string;
  results?: Array<{ quote: string; category: string }>;
  message?: string;
  error?: string;
}
...
export class QuotesService {
  ...
  async getRandomQuote(): Promise<QuoteResult> {
    try {
      const categories = Object.keys(this.quotes) as Array<keyof typeof this.quotes>;
  ...
  async getQuoteByCategory(category: string): Promise<QuoteResult> {
    try {
      ...
      const categoryQuotes = this.quotes[category as keyof typeof this.quotes];
  ...
  async searchQuotes(keyword: string): Promise<QuoteResult> {
    try {
      if (!keyword) {
        return {
          success: false,
      ...
      const results: Array<{ quote: string; category: string }> = [];
      
      Object.keys(this.quotes).forEach(category => {
        this.quotes[category as keyof typeof this.quotes].forEach(quote => {
