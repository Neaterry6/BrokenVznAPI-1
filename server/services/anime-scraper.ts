...
export interface ScrapedStreamingLink {
  quality: string;
  url: string;
  server: string;
  type: 'mp4' | 'hls' | 'm3u8';
...
export class AnimeScraperService {
  ...
  async scrapeGogoAnime(query: string): Promise<{ success: boolean; data?: ScrapedAnimeInfo[]; error?: string; creator: string }> {
    try {
      ...
      const results: ScrapedAnimeInfo[] = [];
  ...
  async scrapeAnimePahe(query: string): Promise<{ success: boolean; data?: ScrapedAnimeInfo[]; error?: string; creator: string }> {
    try {
  ...
  async scrapeNyaaTorrents(query: string): Promise<{ success: boolean; data?: TorrentInfo[]; error?: string; creator: string }> {
    try {
  ...
  async scrapeZoro(query: string): Promise<{ success: boolean; data?: ScrapedAnimeInfo[]; error?: string; creator: string }> {
    try {
  ...
  async scrape9anime(query: string): Promise<{ success: boolean; data?: ScrapedAnimeInfo[]; error?: string; creator: string }> {
    try {
  ...
  async aggregateSearch(query: string): Promise<{ success: boolean; data?: { source: string; results: ScrapedAnimeInfo[] }[]; error?: string; creator: string }> {
    try {
      const sources = [
  ...
  async getStreamingLinks(animeUrl: string): Promise<{ success: boolean; data?: ScrapedStreamingLink[]; error?: string; creator: string }> {
    try {
      // This is a simplified implementation for demo purposes
      // In a real implementation, you would need to handle different video extractors
      const demoLinks: ScrapedStreamingLink[] = [
        {
          quality: '1080p',
