...
export class EnhancedWaifuService {
    ...
    async getWaifuImage(category = 'waifu', type = 'sfw') {
    ...
    async fetchFromAPI(api, category, type) {
        try {
    ...
    parseAPIResponse(data, apiName, category, type) {
        let imageData = null;
        switch (apiName) {
            case 'waifu.im':
    ...
    async getBatchWaifus(count = 5, category = 'waifu', type = 'sfw') {
        try {
            if (count > 20)
                count = 20; // Limit to prevent abuse
            if (count < 1)
                count = 1;
            const promises = Array(count).fill(null).map(() => this.getWaifuImage(category, type));
            const results = await Promise.allSettled(promises);
            const successfulResults = results
                .filter((result) => result.status === 'fulfilled' && result.value.success === true && result.value.data !== undefined)
    ...
    async getHentaiImage(category = 'hentai') {
    ...
    getAvailableCategories() {
        const sfwCategories = new Set();
        const nsfwCategories = new Set();
        const allSfwCategories = [];
        const allNsfwCategories = [];
        this.waifuApis.forEach(api => {
