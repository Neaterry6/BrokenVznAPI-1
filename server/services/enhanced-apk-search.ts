...
export interface EnhancedAppSearchResult {
  success: boolean;
  data?: {
    apps: Array<{
      appName: string;
...
export class EnhancedAPKSearchService {
  ...
  async searchApps(query: string, page: number = 1, limit: number = 10): Promise<EnhancedAppSearchResult> {
    try {
  ...
  // Search APKPure
  private async searchAPKPure(query: string, limit: number): Promise<any[]> {
    try {
  ...
  // Search Uptodown
  private async searchUptodown(query: string, limit: number): Promise<any[]> {
    try {
  ...
  private async searchFDroid(query: string, limit: number): Promise<any[]> {
    try {
      ...
      if (response.data && response.data.packages) {
        response.data.packages.slice(0, limit).forEach((pkg: any) => {
          apps.push({
  ...
  async getPopularApps(category?: string): Promise<EnhancedAppSearchResult> {
    try {
  ...
  async getAppDetails(packageName: string, source?: string): Promise<EnhancedAppSearchResult> {
    try {
      let appDetails = null;
  ...
  // Helper methods
  private async getAPKPureDetails(packageName: string): Promise<any | null> {
    try {
  ...
  private async getUptodownDetails(packageName: string): Promise<any | null> {
    // Similar implementation for Uptodown
    return null; // Placeholder
  }

  private async getFDroidDetails(packageName: string): Promise<any | null> {
    try {
  ...
  private removeDuplicateApps(apps: any[]): any[] {
    const seen = new Set();
  ...
  private extractPackageFromUrl(url: string): string {
