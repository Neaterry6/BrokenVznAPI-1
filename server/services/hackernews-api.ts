...
export interface HackerNewsRequest {
  id?: number;
  type?: 'top' | 'new' | 'best' | 'ask' | 'show' | 'jobs';
  limit?: number;
}
...
export class HackerNewsAPIService {
  ...
  async getStories(request: HackerNewsRequest): Promise<HackerNewsResponse> {
    try {
  ...
  async getItem(request: HackerNewsRequest): Promise<any> {
    try {
      const { id } = request;
