...
export class EnhancedJokesService {
  ...
  async getRandomJoke(): Promise<JokesResponse> {
    try {
  ...
  async getJokeByCategory(category: string): Promise<JokesResponse> {
    try {
      ...
      if (categoryLower.includes('tech') || categoryLower.includes('programming') || categoryLower.includes('computer')) {
        filteredJokes = this.jokes.filter(joke => 
          joke.toLowerCase().includes('computer') ||
        ...
        filteredJokes = this.jokes.filter(joke => 
          joke.includes('?') || joke.includes('Why')
        );
      }
