...
export class PexelsService {
  ...
  private readonly supportedColors = [
  ...
  async searchPhotos(request: PexelsRequest): Promise<PexelsResult> {
    try {
      const {
        query = 'nature',
        orientation,
  ...
  async searchVideos(request: PexelsRequest): Promise<PexelsResult> {
    try {
      const {
        query = 'nature',
        orientation,
  ...
  async getCuratedPhotos(page: number = 1, per_page: number = 15): Promise<PexelsResult> {
    try {
      if (per_page > 80) {
        return {
          success: false,
  ...
  async getPopularVideos(page: number = 1, per_page: number = 15): Promise<PexelsResult> {
    try {
      if (per_page > 80) {
        return {
          success: false,
  ...
  async getPhotoById(id: number): Promise<PexelsResult> {
    try {
      if (!this.apiKey) {
        return {
          success: false,
  ...
  async getVideoById(id: number): Promise<PexelsResult> {
    try {
      if (!this.apiKey) {
        return {
          success: false,
  ...
  async getRandomPhotos(category?: string, count: number = 10): Promise<PexelsResult> {
  ...
  private getDemoPhotosResponse(query: string, per_page: number): PexelsResult {
    const demoPhotos: PexelsPhoto[] = Array.from({ length: Math.min(per_page, 5) }, (_, i) => ({
      id: 1000 + i,
      width: 1920,
      height: 1280,
  ...
  private getDemoVideosResponse(query: string, per_page: number): PexelsResult {
    const demoVideos: PexelsVideo[] = Array.from({ length: Math.min(per_page, 3) }, (_, i) => ({
      id: 2000 + i,
      width: 1920,
      height: 1080,
      duration: 30 + i * 10,
  ...
  private handleError(error: any): PexelsResult {
