...
export interface CategoryAnimeResult {
  success: boolean;
  data?: CategoryAnime[];
  pagination?: {
    currentPage: number;
...
export class AnimeCategoriesService {
  ...
  async getCategories(): Promise<AnimeCategoryResult> {
    try {
      ...
      const categories: AnimeCategory[] = response.data.data.map((genre: any) => ({
      ...
      const predefinedCategories: AnimeCategory[] = [
        {
          id: 'trending',
          name: 'Trending Now',
      ...
      const fallbackCategories: AnimeCategory[] = [
  ...
  async getAnimeByCategory(categoryId: string, page: number = 1, limit: number = 25): Promise<CategoryAnimeResult> {
    try {
      let apiUrl = '';
      ...
      const animeList: CategoryAnime[] = animeData.map((anime: any) => ({
        id: anime.mal_id,
        title: anime.title,
      ...
      const demoAnime: CategoryAnime[] = Array.from({ length: Math.min(limit, 10) }, (_, i) => ({
        id: i + 1,
  ...
  async searchAnimeInCategory(categoryId: string, query: string, page: number = 1): Promise<CategoryAnimeResult> {
    try {
      const params: any = {
        q: query,
        page,
  ...
  async getRandomAnimeFromCategory(categoryId: string, count: number = 10): Promise<CategoryAnimeResult> {
    try {
