...
export interface EnhancedAdultContentResult {
  success: boolean;
  data?: {
    items: Array<{
      id: string;
...
export class EnhancedAdultContentService {
  ...
  async searchBooruContent(query: string, site: string = 'auto', page: number = 1, limit: number = 20): Promise<EnhancedAdultContentResult> {
    try {
      ...
      for (const api of booruApis) {
        if (site !== 'auto' && api.name !== site) continue;
  ...
  private parseBooruResponse(data: any, format: string, site: string): any[] {
    let posts: any[] = [];
  ...
  async searchAdultVideos(query: string, page: number = 1, limit: number = 20): Promise<EnhancedAdultContentResult> {
    try {
      ...
      const videos = response.data.videos.map((video: any) => ({
        ...
        tags: video.keywords ? video.keywords.split(',').map((tag: string) => tag.trim()) : [],
        rating: 'explicit',
        views: video.views || 0,
  ...
  async searchXNXXContent(query: string, page: number = 1): Promise<EnhancedAdultContentResult> {
    try {
  ...
  async getTrendingContent(type: 'images' | 'videos' | 'all' = 'all'): Promise<EnhancedAdultContentResult> {
    try {
  ...
  // Helper methods
  private cleanTitle(tags: string): string {
  ...
  async downloadXNXXVideo(videoId: string): Promise<EnhancedAdultContentResult> {
    try {
