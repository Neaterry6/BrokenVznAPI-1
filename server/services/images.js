...
export class ImagesService {
    ...
    async searchUnsplash(query, limit) {
        try {
    ...
    async searchPixabay(query, limit) {
        try {
    ...
    async searchPexels(query, limit) {
        try {
            ...
            if (response.data && response.data.photos.length > 0) {
                return {
                    success: true,
                    query,
                    images: response.data.photos.map((image) => ({
                        id: image.id.toString(),
    ...
    generatePlaceholderImages(query, limit) {
        const images = [];
