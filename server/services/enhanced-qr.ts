...
export interface EnhancedQRRequest {
  text: string;
  size?: number;
...
export interface EnhancedQRResult {
  success: boolean;
  text?: string;
  qr_url?: string;
  qr_data?: string;
...
export class EnhancedQRService {
  private readonly QR_DIR = '/tmp/qr_codes';
  ...
  async generateQR(request: EnhancedQRRequest): Promise<EnhancedQRResult> {
    try {
      const { 
        text, 
        size = 256, 
  ...
  cleanupOldFiles(maxAgeHours: number = 24): void {
    try {
      if (!existsSync(this.QR_DIR)) return;
