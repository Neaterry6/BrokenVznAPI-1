...
export interface AdultContentSearchRequest {
  query: string;
  page?: number;
  limit?: number;
  tags?: string[];
  rating?: 'safe' | 'questionable' | 'explicit';
  sort?: 'date' | 'score' | 'views' | 'random';
  site?: 'rule34' | 'gelbooru' | 'danbooru' | 'safebooru';
}
...
export interface AdultVideoItem {
  id: string;
  title: string;
  url: string;
  thumbnail: string;
...
export interface AdultContentResult {
  success: boolean;
  data?: {
    items?: AdultContentItem[] | AdultVideoItem[];
    pagination?: {
      currentPage: number;
      totalPages: number;
      totalItems: number;
...
export class AdultContentService {
  ...
  async searchRule34(request: AdultContentSearchRequest): Promise<AdultContentResult> {
    try {
      ...
      for (const apiUrl of fallbackUrls) {
        try {
          ...
          const items: AdultContentItem[] = posts.map((post: any) => ({
            ...
            tags: post.tags ? post.tags.split(' ').filter((tag: string) => tag.length > 0) : [],
  ...
  async searchGelbooru(request: AdultContentSearchRequest): Promise<AdultContentResult> {
    try {
  ...
  async searchHanimeVideos(query: string, page: number = 1): Promise<AdultContentResult> {
    try {
      ...
      const filteredVideos = query 
        ? hentaiVideos.filter((video: any) => 
            video.name?.toLowerCase().includes(query.toLowerCase()) ||
            video.title?.toLowerCase().includes(query.toLowerCase())
          )
        : hentaiVideos;

      const items: AdultVideoItem[] = filteredVideos.slice(0, 20).map((video: any, index: number) => ({
  ...
  private extractVideosFromNuxtData(data: any): any[] {
    const videos: any[] = [];
  ...
  private parseVideosFromHTML(html: string): any[] {
    const videos: any[] = [];
  ...
  async getHanimeVideoDetails(slug: string): Promise<AdultContentResult> {
    try {
      ...
      const streamingUrls = streams.map((stream: any) => ({
      ...
      const videoItem: AdultVideoItem = {
        ...
        quality: streams.length > 0 ? `${Math.max(...streams.map((s: any) => s.height))}p` : '1080p',
        downloadUrl: streamingUrls.length > 0 ? streamingUrls[0].url : undefined
      };

      // Add streaming URLs as additional data
      (videoItem as any).streamingUrls = streamingUrls;
      (videoItem as any).description = hentaiVideo.description;
  ...
  private extractTags(hentaiTags?: any[]): string[] {
    if (!hentaiTags || !Array.isArray(hentaiTags)) return [];
    return hentaiTags.map(tag => tag.text || tag.name || '').filter(tag => tag.length > 0);
  }

  // Random adult content
  async getRandomContent(site: string = 'rule34', limit: number = 10): Promise<AdultContentResult> {
    try {
  ...
  // Get popular tags
  async getPopularTags(site: string = 'rule34'): Promise<AdultContentResult> {
    try {
      const popularTags = [
  ...
  async searchMultipleSites(request: AdultContentSearchRequest): Promise<AdultContentResult> {
    try {
      const promises = [
      ...
      const successfulResults = results
        .filter((result): result is PromiseFulfilledResult<AdultContentResult> => 
      ...
      const allItems: AdultContentItem[] = [];
      successfulResults.forEach(result => {
        if (result.data?.items) {
          allItems.push(...(result.data.items as AdultContentItem[]));
        }
      });
  ...
  async safeModeSearch(query: string, page: number = 1): Promise<AdultContentResult> {
    try {
      return this.searchRule34({
