...
export class AnimeImageSearchService {
    ...
    async searchAnimeImages(query, page = 1, limit = 10) {
        try {
    ...
    parseBooruResponse(data, format, siteName) {
        let posts = [];
        try {
            switch (format) {
                case 'gelbooru':
    ...
    getFallbackResults(query, page, limit) {
    ...
    async searchCharacter(characterKey) {
        try {
            const searchTerm = this.animeCharacters[characterKey];
            if (!searchTerm) {
                return {
                    success: false,
    ...
    async searchGenre(genreKey, limit = 10) {
        try {
            const searchTerm = this.animeGenres[genreKey];
            if (!searchTerm) {
                return {
                    success: false,
    ...
    async getRandomAnimeImages(count = 5) {
        try {
            if (count > 15)
                count = 15; // Limit for performance
            if (count < 1)
                count = 1;
    ...
    async searchAnimeWallpapers(query = 'anime', resolution = '1920x1080') {
        try {
